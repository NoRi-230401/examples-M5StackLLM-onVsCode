 ...
 * "SerialTextAssistant.ino" を日本語の表示ができるように変更 by NoRi
 *  シリアルモニターから質問を送るときに、行末を CRLF に指定する
 */
#include <Arduino.h>
#include <M5Unified.h>
...
void setup()
{
...
#if defined(ENABLE_SD_UPDATER)
    SDU_lobby("SerialTextAssistant-jp");  //-- for SD-Updater ---
#endif
   ...
   /* Init usb serial */
    ...
    M5.Display.printf(">> シリアルポート経由で質問してね\n");
    M5.Display.setTextColor(TFT_GREEN);

    // M5.Display.printf("e.g. \nHi, What's your name?\n");
    // M5.Display.printf("(end with CRLF \\r\\n)\n\n");
    M5.Display.printf("（例）あなたの名前を教えて？\n");
    M5.Display.printf("(行末を CRLF に設定)\n\n");
}
